...
esp_err_t helper_get_fbd(httpd_req_t *req, const char *filepath)
{
    ...
    do {
        ...
        chunksize = fread(chunk, 1, CONFIG_SMOPLA_HTTP_SCRATCHPAD_SIZE, fd);
...
{    
    ...
    if(remaining>=CONFIG_SMOPLA_HTTP_SCRATCHPAD_SIZE)
        return ESP_FAIL;
    ...
    while (remaining > 0) {
        ...
        if ((ret = httpd_req_recv(req, (char*)buf,  MIN(remaining, CONFIG_SMOPLA_HTTP_SCRATCHPAD_SIZE))) <= 0) {
...
esp_err_t handle_get_fbd(httpd_req_t *req){
    ...
    if(size>=CONFIG_SMOPLA_HTTP_SCRATCHPAD_SIZE)
        return ESP_FAIL;
    uint8_t *buf= static_cast<uint8_t*>(httpd_get_global_user_ctx(req->handle));
...
esp_err_t helper_post_fbd(httpd_req_t *req, const char *filepath, bool overwrite){
    ...
    while (remaining > 0) {
        ...
        if ((received = httpd_req_recv(req, buf, MIN(remaining, CONFIG_SMOPLA_HTTP_SCRATCHPAD_SIZE))) <= 0) {
